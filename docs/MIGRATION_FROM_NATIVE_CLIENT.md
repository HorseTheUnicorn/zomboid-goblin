...
The replacement is a server-side NPC body created through the vanilla Build 42
server Lua surface. The only gameplay command is a typed
`command.npc_action` message, and the body-specific boundary is
`VanillaNpcAdapter.lua`. Existing saves remain in scope, but the server must be
tested after each mod update and unsupported engine capabilities must remain
disabled.
