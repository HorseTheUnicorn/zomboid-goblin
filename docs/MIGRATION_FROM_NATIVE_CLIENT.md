...
The server publishes a bounded snapshot, and each joining client creates a
local `HumanSurvivor` Java actor from the normal survivor descriptor API. The
actor is neither a player nor a zombie. GoblinSurvivor owns the actor identity,
snapshot authority, client rendering, chat, persistence, and command policy.
Joining clients need GoblinSurvivor for the client actor and chat relay, but
no separate NPC framework or client account is required.
...
checks pass, the standalone package can be staged for one guarded production
